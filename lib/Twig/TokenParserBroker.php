 ...
 */
class TwigKagg_TokenParserBroker implements TwigKagg_TokenParserBrokerInterface
{
    protected $parser;
    protected $parsers = [];
    protected $brokers = [];
    ...
    /**
     * @param array|\Traversable $parsers                 A \Traversable of TwigKagg_TokenParserInterface instances
     * @param array|\Traversable $brokers                 A \Traversable of TwigKagg_TokenParserBrokerInterface instances
    ...
    public function __construct($parsers = [], $brokers = [], $triggerDeprecationError = true)
    {
        ...
        foreach ($parsers as $parser) {
            if (!$parser instanceof TokenParserInterface) {
                throw new \LogicException('$parsers must a an array of TwigKagg_TokenParserInterface.');
            }
            $this->parsers[$parser->getTag()] = $parser;
        }
        foreach ($brokers as $broker) {
            if (!$broker instanceof TwigKagg_TokenParserBrokerInterface) {
                throw new \LogicException('$brokers must a an array of TwigKagg_TokenParserBrokerInterface.');
            }
            $this->brokers[] = $broker;
        }
    }
