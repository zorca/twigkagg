...
namespace TwigKagg\Tests\NodeVisitor;
...
class OptimizerTest extends \PHPUnit\Framework\TestCase
{
    ...
    {
        $env = new Environment($this->createMock('\TwigKagg\Loader\LoaderInterface'), ['cache' => false, 'autoescape' => false]);

        $stream = $env->parse($env->tokenize(new Source('{{ block("foo") }}', 'index')));

        $node = $stream->getNode('body')->getNode(0);

        $this->assertInstanceOf('\TwigKagg\Node\Expression\BlockReferenceExpression', $node);
    ...
    public function testRenderParentBlockOptimizer()
    {
        ...
        $this->assertInstanceOf('\TwigKagg\Node\Expression\ParentExpression', $node);
        $this->assertTrue($node->getAttribute('output'));
    }
    ...
    {
        $env = new Environment($this->createMock('\TwigKagg\Loader\LoaderInterface'), ['cache' => false]);
    ...
    public function checkForConfiguration(\TwigKagg_NodeInterface $node = null, $target, $withLoop)
    {
        if (null === $node) {
            return;
        }
