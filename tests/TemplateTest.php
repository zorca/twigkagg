...
class TemplateTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function getAttributeExceptions()
    {
        return [
            ...
            ['{{ array_access["a"] }}', 'Key "a" in object with ArrayAccess of class "TwigKagg\Tests\TemplateArrayAccessObject" does not exist in "%s" at line 1.'],
            ...
            ['{{ array_access.a }}', 'Neither the property "a" nor one of the methods "a()", "geta()"/"isa()" or "__call()" exist and have public access in class "TwigKagg\Tests\TemplateArrayAccessObject" in "%s" at line 1.'],
            ['{% from _self import foo %}{% macro foo(obj) %}{{ obj.missing_method() }}{% endmacro %}{{ foo(array_access) }}', 'Neither the property "missing_method" nor one of the methods "missing_method()", "getmissing_method()"/"ismissing_method()" or "__call()" exist and have public access in class "TwigKagg\Tests\TemplateArrayAccessObject" in "%s" at line 1.'],
    ...
    public function testGetAttributeWithTemplateAsObject()
    {
        ...
        $twig = new Environment($this->createMock('TwigKagg\Tests\TemplateTestLoaderInterface'));
        //$twig = new Environment($this->createMock('\TwigKagg\Loader\LoaderInterface', '\TwigKagg\Loader\SourceContextLoaderInterface'));

        $template = new TemplateForTest($twig, 'index.twig');
        $template1 = new TemplateForTest($twig, 'index1.twig');

        $this->assertInstanceOf('\TwigKagg\Markup', $template->getAttribute($template1, 'string'));
        $this->assertEquals('some_string', $template->getAttribute($template1, 'string'));

        $this->assertInstanceOf('\TwigKagg\Markup', $template->getAttribute($template1, 'true'));
        $this->assertEquals('1', $template->getAttribute($template1, 'true'));

        $this->assertInstanceOf('\TwigKagg\Markup', $template->getAttribute($template1, 'zero'));
        $this->assertEquals('0', $template->getAttribute($template1, 'zero'));

        $this->assertNotInstanceof('\TwigKagg\Markup', $template->getAttribute($template1, 'empty'));
    ...
    {
        $template = new TemplateForTest(new Environment($this->createMock('\TwigKagg\Loader\LoaderInterface')));
    ...
    {
        $template = new TemplateForTest(new Environment($this->createMock('\TwigKagg\Loader\LoaderInterface'), ['strict_variables' => true]));

        if ($defined) {
            $this->assertEquals($value, $template->getAttribute($object, $item, $arguments, $type));
        } else {
            $this->expectException('\TwigKagg\Error\RuntimeError');
