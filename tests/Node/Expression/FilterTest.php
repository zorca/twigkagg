...
class FilterTest extends NodeTestCase
{
    ...
    public function getTests()
    {
        ...
        $environment->addFilter(new TwigFilter('barbar', 'TwigKagg\Tests\Node\Expression\twig_tests_filter_barbar', ['needs_context' => true, 'is_variadic' => true]));
        ...
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_filter_barbar($context, "abc")', $environment];

        $node = $this->createFilter($string, 'barbar', ['foo' => new ConstantExpression('bar', 1)]);
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_filter_barbar($context, "abc", null, null, ["foo" => "bar"])', $environment];

        $node = $this->createFilter($string, 'barbar', ['arg2' => new ConstantExpression('bar', 1)]);
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_filter_barbar($context, "abc", null, "bar")', $environment];
        ...
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_filter_barbar($context, "abc", "1", "2", [0 => "3", "foo" => "bar"])', $environment];
    ...
    {
        $this->expectException('\TwigKagg\Error\SyntaxError');
