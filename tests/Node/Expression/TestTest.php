...
class TestTest extends NodeTestCase
{
    ...
    public function getTests()
    {
        ...
        $environment->addTest(new TwigTest('barbar', 'TwigKagg\Tests\Node\Expression\twig_tests_test_barbar', ['is_variadic' => true, 'need_context' => true]));
        ...
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_test_barbar("abc")', $environment];

        $node = $this->createTest($string, 'barbar', ['foo' => new ConstantExpression('bar', 1)]);
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_test_barbar("abc", null, null, ["foo" => "bar"])', $environment];

        $node = $this->createTest($string, 'barbar', ['arg2' => new ConstantExpression('bar', 1)]);
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_test_barbar("abc", null, "bar")', $environment];
        ...
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_test_barbar("abc", "1", "2", [0 => "3", "foo" => "bar"])', $environment];
