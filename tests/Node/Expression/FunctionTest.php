...
class FunctionTest extends NodeTestCase
{
    ...
    public function getTests()
    {
        ...
        $environment->addFunction(new TwigFunction('barbar', 'TwigKagg\Tests\Node\Expression\twig_tests_function_barbar', ['is_variadic' => true]));
        ...
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_function_barbar()', $environment];

        $node = $this->createFunction('barbar', ['foo' => new ConstantExpression('bar', 1)]);
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_function_barbar(null, null, ["foo" => "bar"])', $environment];

        $node = $this->createFunction('barbar', ['arg2' => new ConstantExpression('bar', 1)]);
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_function_barbar(null, "bar")', $environment];
        ...
        $tests[] = [$node, 'TwigKagg\Tests\Node\Expression\twig_tests_function_barbar("1", "2", [0 => "3", "foo" => "bar"])', $environment];
